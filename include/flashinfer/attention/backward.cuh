...
__device__ __forceinline__ void page_produce_kv(smem_t<swizzle_mode> smem, uint32_t* smem_offset,
  ...
  constexpr uint32_t head_dim = NUM_FRAGS_D * 16; // each frag has 16 dims?
  constexpr uint32_t num_warps = NUM_WARPS_Q * NUM_WARPS_KV;
...
#pragma unroll
    for (uint32_t fd = 0; fd < NUM_FRAGS_D; ++fd) {
      uint32_t b_frag[4]; // load V into b_frag
      if constexpr (sizeof(DTypeKV) == 1) {
        uint32_t b_frag_f8[2];
        if (fd % 2 == 0) {
...
__launch_bounds__(NUM_WARPS_Q* NUM_WARPS_KV* WARP_SIZE) void BackwardWithPagedKVCacheKernel(
  ...
  if constexpr (std::is_same_v<DTypeQ, nv_bfloat16>) {
    ...
    // q loaded from gmem to smeme, offsets for q/o calculated, m/d/o inited

    if constexpr (POS_ENCODING_MODE == PosEncodingMode::kRoPELlama) { // apply rotary encoding
      if (q_offset == nullptr) {
        q_smem_inplace_apply_rotary<NUM_WARPS_Q, NUM_WARPS_KV, NUM_FRAGS_Q, NUM_FRAGS_D,
                                    swizzle_mode_q, DTypeQ>(
            qo_packed_idx_base, qo_len, kv_len, group_size, &qo_smem, &q_smem_offset_r, rope_freq);
    ...
    q_smem_inplace_transform<NUM_WARPS_Q, NUM_WARPS_KV, NUM_FRAGS_Q, NUM_FRAGS_D, swizzle_mode_q>(
        params, variant, &qo_smem); // transform q based on attention variant.
    ...
    constexpr uint32_t kv_frag_cols = swizzle_mode_kv == SwizzleMode::k128B ? 8 : 4;
    smem_t<swizzle_mode_kv> k_smem(smem +
                                   (NUM_WARPS_Q * NUM_FRAGS_Q * sizeof(DTypeQ)) * 16 * head_dim), // smem + size of q (total)?
        v_smem(smem + (NUM_WARPS_Q * NUM_FRAGS_Q * sizeof(DTypeQ) +
                       NUM_WARPS_KV * NUM_FRAGS_KV * sizeof(DTypeKV)) *
                          16 * head_dim); // smem + size of q + size of k
    size_t kv_offset[NUM_FRAGS_KV * (swizzle_mode_kv == SwizzleMode::k128B ? 4 : 2) / NUM_WARPS_Q]; // wth

    uint32_t k_smem_offset_r = k_smem.get_permuted_offset<channel_size_128b_kv>( // channel_size_128b_kv = num 128b elem for each k/v
                 get_warp_idx_kv<NUM_WARPS_Q, NUM_WARPS_KV>() * NUM_FRAGS_KV * 16 +
                     8 * (lane_idx / 16) + lane_idx % 8,
                 (lane_idx % 16) / 8),
             v_smem_offset_r = v_smem.get_permuted_offset<channel_size_128b_kv>(
